...
window.addEventListener('load', () => {
    ...
    // Typewriter effect for neon message
    const messageElement = document.getElementById('neon-message');
    const fullMessage = "Anneler gününüz kutlu olsun";
    let currentIndex = 0;

    function typeWriter() {
        if (currentIndex < fullMessage.length) {
            messageElement.textContent += fullMessage.charAt(currentIndex);
            currentIndex++;
            setTimeout(typeWriter, 150); // Harf arası gecikme
        } else {
            // Yazma bitti
        }
    }

    // Çiçek animasyonlarından sonra typewriter başlat (5 saniye sonra)
    setTimeout(() => {
        messageElement.style.opacity = '1'; // Görünür yap
        typeWriter();
    }, 5000);
});
